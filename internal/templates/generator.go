...
func GenerateBoilerplate(meta *ProjectMetadata) error {
	...
	// 🟢 FORCED EXPLICIT ROUTING MAPPING FOR ALL BACKEND ENGINE FRAMEWORKS
	switch meta.Backend {
	case "Node.js (Express)":
		meta.BackendPort = 8080      // Standardized to match the index.js fallback configuration
		meta.BackendHealthPath = "/" // Express skeletons respond directly to the root path
	case "Python (Django)":
		meta.BackendPort = 8000
		meta.BackendHealthPath = "/" // Django boilerplate runs on root path natively
	case "Rust (Actix-web)":
	...
	// 2. Launch the Automated Backend CLIs (They now safely use the corrected meta values!)
	switch meta.Backend {
	case "Python (Django)":
	...
	if meta.Frontend != "None (Pure Backend API)" {
		...
		// ⚡ AUTOMATED FRAMEWORK CONFIGURATION INJECTORS FOR NGINX COMPATIBILITY
...
const nextConfig = {
  output: 'export', // Forces static HTML export to /app/out
  images: {
...
`
			_ = os.WriteFile(nextConfigPath, []byte(nextConfigContent), 0644)

		} else if strings.Contains(meta.Frontend, "Svelte") {
			svelteConfigPath := filepath.Join(frontendPath, "svelte.config.js")
			svelteConfigContent := `import adapter from '@sveltejs/adapter-static';
...
const config = {
	...
	kit: {
		adapter: adapter({
			pages: 'dist', // Aligns directly with /app/dist in your Dockerfile
			assets: 'dist',
			fallback: '404.html',
			precompress: false,
			strict: true
...
`
			_ = os.WriteFile(svelteConfigPath, []byte(svelteConfigContent), 0644)

			routesDir := filepath.Join(frontendPath, "src", "routes")
			_ = os.MkdirAll(routesDir, 0755)
			pageJsPath := filepath.Join(routesDir, "+page.js")
			_ = os.WriteFile(pageJsPath, []byte("export const prerender = true;\n"), 0644)
		}

		// Adjust operational frontend target ports
		...
		frontendK8sVars := k8s.K8sManifestVars{
			...
			ContainerPort: 80, // Securely locks Nginx container target monitoring
...
`
	...
	// 🐳 Invoke the Dockerfile builder factory seamlessly
	// 🟢 FIXED BY DEREFERENCING THE POINTER VALUE NATIVELY HERE
	err := generateDynamicDockerfile(basePath, *meta)
	if err != nil {
		fmt.Printf("⚠️  Dockerfile compilation skipped: %v\n", err)
	} else {
		fmt.Printf("🐳 Dockerfile generated successfully in %s\n", basePath)
	}
...
func generateDynamicDockerfile(basePath string, meta ProjectMetadata) error {
	...
	if meta.Frontend != "None (Pure Backend API)" {
		isNextJS := strings.Contains(meta.Frontend, "Next.js")
		frontendBuildDir := "/app/dist"
		if isNextJS {
			frontendBuildDir = "/app/out"
		}

		dockerfileContent += fmt.Sprintf(`# --- Stage 1: Frontend Dependency & Build Pipeline ---
...
COPY --from=frontend-builder %s /usr/share/nginx/html
EXPOSE 80
...
`, meta.BackendPort, meta.BackendPort)
	...
	case "Node.js (Express)":
		dockerfileContent += fmt.Sprintf(`# --- Stage 3: Node.js Express Backend Engine ---
